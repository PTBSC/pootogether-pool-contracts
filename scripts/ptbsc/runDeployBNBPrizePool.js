...
async function main() {
  ...
  console.log("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
  console.log("PoolTogether Pool Contracts - Transactions Script")
  console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")

  const { getNamedAccounts, getChainId, ethers } = hardhat
  const { deployer, rng, pool, BNBTokenAddr, vBNBTokenAddr } = await getNamedAccounts()
  ...
  const vBNBToken = await ethers.getContractAt('IERC20Upgradeable', vBNBTokenAddr, signer)
  ...
  let multipleWinnersConfig = {
    ...
    ticketName: "Poo Venus BNB",
    ticketSymbol: "PvBNB",
    sponsorshipName: "SponsorPooB",
    sponsorshipSymbol: "SPOOB",
    ticketCreditLimitMantissa: toWei('0'),
    ticketCreditRateMantissa: toWei('0'), // 10 day cooldown for 1% fee
    numberOfWinners: 3
  }
  let prizePoolConfig = {
    cToken: vBNBToken.address, //vBUSD TESTNET
  ...
  const vBNBTokenFaucet = await getProxy(tx, signer)
  console.log([pooToken.address, ticket, dripRate,vBNBTokenFaucet])
  console.log(`Created TokenFaucet at ${vBNBTokenFaucet} => ${tx.hash}`)
  tx = await vbusdPrizeStrategy.setTokenListener(vBNBTokenFaucet)
  console.log(`Set Token Listener at ${vBNBTokenFaucet} => ${tx.hash}`)

// Move to disburse
  tx = await pooToken.transfer(vBNBTokenFaucet, allocatedToken) // pootoken
  console.log(`Transfer  ${vBNBTokenFaucet} => ${tx.hash}`)
